...
typedef struct SpiFns
{
  ...
  void (*read_write)(Spi *self, const uint8_t *src, uint8_t *dst, uint32_t len);
...
static inline void Spi_read_write(Spi *self, const uint8_t *src, uint8_t *dst, uint32_t length)
{
  self->fns->read_write(self, src, dst, length);
}
#endif // SPI_H
