...
{
    uint32_t writtenBytes = flashWriteBitstream(&flashConfig, task_context->input_data, task_context->input_data_len);
    task_context->task_services.send_return(&task_context->task_services, 0, 0, false);
}
...
void write_to_flash_from_remote(TaskContext *task_context)
{
    ...
    {
        UserDataStruct *user_data = (UserDataStruct *)task_context->user_data;
        uint16_t writtenBytes = flashWritePage(&flashConfig, task_context->step_counter * flashConfig.bytesPerPage * (user_data->target_sector * flashConfig.bytesPerSector), task_context->input_data, flashConfig.bytesPerPage);
    }
...
void predict(TaskContext *task_context)
{
    ...
    FpgaMiddleware_init(fpga_middleware);
    FpgaMiddleware_enableUserLogic(fpga_middleware);
    FpgaMiddleware_write(fpga_middleware, task_context->input_data, USER_LOGIC_START_ADDRESS, task_context->input_data_len);
    ...
    FpgaMiddleware_read(fpga_middleware, task_context->output_data, USER_LOGIC_START_ADDRESS, task_context->input_data_len);
