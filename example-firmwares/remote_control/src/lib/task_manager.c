...
{
    task_manager.ctx.outgoing_rb = outgoing_rb;

    for (int i = 0; i < MAX_TASKS; i++)
    {
        task_manager.task_pool[i].id = i;
        task_manager.task_pool[i].status = TASK_STATUS_IDLE;
        memset(task_manager.task_pool[i].input_data, 0, TASK_BUFFER_LEN_BYTE);
        task_manager.task_pool[i].input_data_len = 0;
        memset(task_manager.task_pool[i].output_data, 0, TASK_BUFFER_LEN_BYTE);
        task_manager.task_pool[i].output_data_len = 0;
        task_manager.task_pool[i].fn_id = 0;
        task_manager.task_pool[i].outgoing_rb = outgoing_rb;
    }
}
...
void process_tasks(RingBuffer *task_rb, RingBuffer *outgoing_rb)
{
    ...
    while (ringbuffer_pop(task_rb, &task))
    {
        ...
        task->run(task, &task_manager.ctx);
    }
}
