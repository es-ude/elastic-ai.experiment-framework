...
void process_tasks(RingBuffer *task_rb, RingBuffer *outgoing_rb, TaskManager *task_manager)
{
    ...
    for (int i = 0; i < MAX_TASKS; i++) // Check internally queued tasks with timer
    {
        ...
        {
            uint64_t current_time_ms = transport_get_current_time_ms();
            if (current_time_ms - task->ctx.exe_last_ms >= task->ctx.exe_interval_ms)
            {
                task->ctx.exe_last_ms = current_time_ms;
                task->funcs->handle(&task->ctx);
            }
        }
    }
