...
void test_receive_datachunk_accumulates_data(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8(0x42, t->input_data[0]);
}

/* -------- process_tasks -------- */

static void dummy_run(Task *task, TaskContext *ctx)
{
    task->output_data[0] = 1;
}
...
void test_process_tasks_executes_task(void)
{
    ...
    TEST_ASSERT_EQUAL_PTR(dummy_run, t->run);
    fflush(stdout);

    process_tasks(&task_rb, &out_rb);

    TEST_ASSERT_EQUAL_INT(1, t->output_data[0]);
}
