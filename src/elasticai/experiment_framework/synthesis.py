...
class ConnectionWrapperForFabric:
...
class ConnectionWrapperForInvoke:
...
def main(
    ...
    """
    if host == "":
        connection = ConnectionWrapperForInvoke(
            _invContext(), verbosity=Verbosity.ONLY_ERRORS if quiet else Verbosity.ALL
        )
    else:
        connection = ConnectionWrapperForFabric(
