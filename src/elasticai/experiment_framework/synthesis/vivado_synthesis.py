...
def _run_synthesis(
    ...
    logger.info("uploading %s", str(src_dir.absolute()))
    ...
    with ExitStack() as ctx_stack:
        ...
        elif str(out_path).endswith(".zip"):
            raise ValueError(f"unsupported output path format {str(out_path)}")
        else:
            if out_path.exists() and not len(list(out_path.iterdir())):
        ...
        with TemporaryDirectory(suffix="synth_server") as tmp_dir:
            logger.info("preparing files in %s", str(tmp_dir))
            tmp_dir = Path(tmp_dir)
            _write_tcl_script(
                tmp_dir,
                project_name=project_name,
                part_number=_fpga_model_for_platform[TargetPlatforms(config.target)],
        ...
        with connection.cd(config.remote_working_dir):
            ...
            cmd = "{vivado} -mode tcl -source {srcs}/{tcl_script}".format(
                vivado=config.vivado_path,
                srcs=_SRCS_FILE_BASE_NAME,
                tcl_script=_TCL_SCRIPT_NAME,
            )
            logger.info(f"starting vivado implementation run:\n\t{cmd}")
            connection.run(cmd)
            ...
            for cmd in [
                ...
                connection.run(cmd)
            connection.get(
                f"{str(config.remote_working_dir)}/results.tar.gz",
                str(target_file),
            )
