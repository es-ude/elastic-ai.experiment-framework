...
class CachedSynthesis[S: SynthesisConfig](SynthesisStrategy):
    ...
    def synthesize(
        ...
        cache_dir: str | Path = "$XDG_CACHE_DIR/eaixp/synthesis/",
    ) -> Path:
    ...
    def _normalize_cache_dir(self, cache_dir: str | Path) -> Path:
        if isinstance(cache_dir, str):
            ...
            for i, part in enumerate(parts):
                if "$" in part:
                    parts[i] = os.environ[part.lstrip("$")]
            cache_dir = Path("/".join(parts))
        return cache_dir
...
def load_synthesis_config_from_env[T: SynthesisConfig](
    ...
    if env is None:
        env = os.environ
    default = dataclasses.asdict(SynthesisConfig())
    ...
    def get(key: str) -> str | int | bool:
        value: str | int | bool = env.get(f"SYNTH_{key.upper()}", default.get(key, ""))
        _type = fields[key].type
        _type_map = {"str": str, "int": int, "bool": _bool}
