...
class Connection:
    ...
    def __init__(self, transportType: TransportType, host=None, port=None, serial_port=None, baudrate=115200):
    ...
    async def _create_connection(self, loop):
        if self._transport_type == TransportType.TCP:
            factory = lambda: InternalTransportProtocolTCP(self._queue, self._on_lost)
            _, self._protocol = await loop.create_connection(
                factory, self._host, self._port
            )

        else:
            raise NotImplementedError(f"transport type {self._transport_type} not supported yet")
