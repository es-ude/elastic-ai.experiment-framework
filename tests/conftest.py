...
class ShortNameFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        name = self.DISPLAY_NAMES.get(record.name, record.name)
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if plugin.log_cli_handler is not None:
        plugin.log_cli_handler.setFormatter(formatter)

    if plugin.log_file_handler is not None:
        plugin.log_file_handler.setFormatter(formatter)
