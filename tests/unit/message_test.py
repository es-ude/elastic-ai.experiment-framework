...
from elasticai.experiment_framework.remote_control.message_validators import (
    validate_payload,
)
...
class TestMessageValidation:
    ...
    def test_valid_payloads(self, command, payload):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (Command.RETURN, b"12"),
            (Command.NACK, b""),
            (Command.NACK, b"\x01\x02"),
        ],
    )
    def test_invalid_payload_lengths(self, command, payload):
        with pytest.raises(InvalidCommandPayloadError):
            validate_payload(command, payload)

    def test_nack_rejects_invalid_error_code(self):
        with pytest.raises(InvalidCommandPayloadError, match="Invalid NACK error code"):
            validate_payload(Command.NACK, b"\xff")
